...
class InvoiceM extends CI_Model {
      ...
      function get_payments($property_id, $flat_no, $month){
        $sql = " SELECT sum(payment.amount) as amount, payment.payment_date from payment, tenants where tenants.property_id=$property_id and tenants.property_id=payment.property_id and tenants.flat_no=payment.flat_no and tenants.flat_no=$flat_no and tenants.status = 1 and  payment.`payment_date` LIKE '$month%'";
        $query = $this->db->query($sql);
        return $query->result_array();
      }
